...
enum WorkItemLink {
  /// `.dcdp/config.toml` on the clone's main worktree, falling back to the user's
  /// Settings defaults (which themselves fall back to CASDevOps/CA Entrega).
  ...
  /// it needs the project segment to resolve, exactly as the team's own
  /// `update_related_workitem_with_build.rb` builds it.
